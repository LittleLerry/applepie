...
And it turns out that result is not even good. generated image have strong locality, let me add some attn in the midblock to see if works
"""
import torch
...
class FNN(nn.Module):
    ...
    def forward(self, x):
        ...
        return self.ffm(x)
# TODO
# to be modified with K, Q, V from the conditions!
class MHA(nn.Module):
...
class block(nn.Module):
    def __init__(self, d_model, num_heads, d_ff):
        super().__init__()
        self.attn = sublayer(MHA(d_model, num_heads), d_model)
        self.fnn = sublayer(FNN(d_model, d_ff), d_model)
    def forward(self,x):
        x = self.attn(x)
        return self.fnn(x)

class attn_layers(nn.Module):
    def __init__(self, seq_len, channels, num_heads):
        super().__init__()
        self.learned_pos_emd = nn.Parameter(torch.randn(size=(seq_len, channels))) # (H*W, C)
        self.blocks = nn.Sequential(*[block(channels, num_heads, 2 * channels) for _ in range(3)])
        self.channels = channels
        self.seq_len = seq_len
    
    def forward(self, x: torch.Tensor, t, y): # (*, C, H, W), timestamps, conditions
        shape = x.shape[:-3]
        H = x.shape[-2]
        W = x.shape[-1]
        x = x.view(-1, self.channels, self.seq_len).transpose(-1, -2) + self.learned_pos_emd # (*, C, H, W) -> (N, C, H*W) -> (N, H*W, C)
        for attn_block in self.blocks:
            x = attn_block(x)
        return x.transpose(-1,-2).view(*shape, -1, H, W).contiguous() # (N, H*W, C) -> (*, C, H, W)
# =============================================================================
# ===================================NEW=======================================
# =============================================================================
...
class midcoder(nn.Module):
    def __init__(self, in_channels, d_t, d_y):
        super().__init__()
        self.bottleneck = nn.Sequential(residual_layer(in_channels, d_t, d_y), attn_layers(32*32, in_channels, 8) ,residual_layer(in_channels, d_t, d_y))
    ...
    for e in range(epoch):
        ...
        if ((e) % sample_epoch_interval == 0):
            with torch.inference_mode():
                ...
                # y = torch.zeros(size=(num_samples,), device=device, dtype=torch.int64) # integer conditioned labels
                y = torch.randint(0, num_labels+1, size=(num_samples,), device=device, dtype=torch.int64) # let's try different conditions!
        ...
        if ((e) % save_epoch_interval == 0) and (rank == 0):
            torch.save(ddp_model.module.state_dict(), f"./ldm_ckpt/v{version}/{e}.pt")
        slr.step()
        dist.barrier()
        # ===== done epoch loop ======
    cleanup()
...
def get_conf():
    ...
    conf = {
        ...
        "sample_epoch_interval" : 32,
        "save_epoch_interval": 32,
        "num_samples_per_gpu": 2,
        "steps": 256,
        "guidance_scale": 1.0,
        "sampling_output_dir": f"./ldm_output/v{version}",
        "world_size": 8,
        "inference_steps": 256,
        "version": version,
    }
