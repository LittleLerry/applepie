...
def run_compute_group_normalized_rewards(
    ...
    raw_rewards_dict = [reward_fn(r, a) for (r, a) in zip(rollout_responses, repeated_ground_truths)]
    raw_rewards = torch.tensor([r["reward"] for r in raw_rewards_dict])
    # get adv
    advs = raw_rewards - raw_rewards.view(-1, group_size).mean(dim=-1, keepdim=False).repeat_interleave(repeats = group_size)
    if normalize_by_std:
        d = raw_rewards.view(-1, group_size).std(dim=-1, keepdim=False).repeat_interleave(repeats = group_size) + advantage_eps
        advs /= d
    
    return (advs, raw_rewards, {"mean_rewards": raw_rewards.mean(), "std_rewards": raw_rewards.std()})
...
def run_get_response_log_probs(
    ...
    result_dict = {}
    result_dict["log_probs"] = log_probs[torch.arange(b)[:, None], torch.arange(s)[None, :], labels]
...
def run_compute_grpo_clip_loss(
    ...
    Args:
        advantages: torch.Tensor of shape (batch_size,): 
    ...
    result = - torch.minimum(ratio * advantages[:, None], torch.clip(ratio, 1 - cliprange, 1 + cliprange) * advantages[:, None])

    return (result, {"clip_mask": clipped_mask})
...
def run_grpo_microbatch_train_step(
    ...
    # calc loss
    loss =  run_masked_mean(loss, response_mask, dim=-1).mean() / gradient_accumulation_steps

    # calc clip ratio
    if "clip_mask" in metadata:
        metadata["clip_ratio"] = run_masked_mean(metadata["clip_mask"] * 1.0, response_mask, dim=-1).mean()
