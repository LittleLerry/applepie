...
def train(rank: int, world_size: int, conf: dict, epoch: int, untrained_para: str, trained_para: str): 
    ...
    for _ in range(epoch):
        for (_, data) in enumerate(get_non_random_train_data(rank, world_size, conf)):
            ...
            # ====================option 1=======================
            def none_helper(model):
                for param in model.parameters():
                    if param.grad is not None:
                        yield param
            grads = [param.grad for param in none_helper(model)]
            ...
            for (p, reduced) in zip(none_helper(model), unflattened):
                p.grad = reduced / (world_size)
            # ====================option 2=======================
            """
            # def grad_itr
            for param in model.parameters():
...
option 1:
Training for the model has been done. avg_t:7.109119415283203, avg_sync_t:0.561648964881897, sync_t/t=7.900401592254639%
...
if __name__ == "__main__":
    ...
    # conf = {"d_model":2560, "d_ff": 10240, "num_layers" : 32, "num_heads": 32, "vocab_size": 10000, "context_length": 128, "rope_theta": 0.5} OOM
    conf = {"d_model":1600, "d_ff": 6400, "num_layers" : 48, "num_heads": 25, "vocab_size": 10000, "context_length": 128, "rope_theta": 0.5}
    # conf = {"d_model":768, "d_ff": 3072, "num_layers" : 12, "num_heads": 12, "vocab_size": 1000, "context_length": 128, "rope_theta": 0.5} debug
